...
func TestExecute(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedErr: "fail get DataUploadResult for restore: testRestore: no DataUpload result cm found for testBackup",
		},
		{
			...
			pvc:              builder.ForPersistentVolumeClaim("velero", "testPVC").ObjectMeta(builder.WithAnnotations(util.VolumeSnapshotRestoreSize, "10Gi")).Result(),
			dataUploadResult: builder.ForConfigMap("velero", "testCM").Data("uid", "{}").ObjectMeta(builder.WithLabels(velerov1api.BackupNameLabel, "testBackup", util.PVCNamespaceNameLabel, "velero.testPVC")).Result(),
			...
			if tc.name == "Restore from VolumeSnapshot" {
				...
				tc.expectedDataDownload = &velerov1api.SnapshotRestore{
					...
					ObjectMeta: metav1.ObjectMeta{
						...
						Namespace:    "velero",
						Labels: map[string]string{
							util.AsyncOperationIDLabel:   "uid.",
				...
				tc.expectedPVC.Spec.Selector = &metav1.LabelSelector{
					MatchLabels: map[string]string{util.DynamicPVRestoreLabel: "velero.testPVC"},
				}
			}
